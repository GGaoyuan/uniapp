...
const uni_modules_limeShared_base64ToArrayBuffer_index = require("../base64ToArrayBuffer/index.js");
const uni_modules_limeShared_pathToBase64_index = require("../pathToBase64/index.js");
const uni_modules_limeShared_isBase64_index = require("../isBase64/index.js");
const uni_modules_limeShared_isString_index = require("../isString/index.js");
class EXIF {
  constructor() {
    __publicField(this, "isXmpEnabled", false);
    ...
    __publicField(this, "StringValues", {
      ExposureProgram: {
        0: "Not defined",
        1: "Manual",
        2: "Normal program",
  ...
  getData(img, callback) {
    ...
    if (uni_modules_limeShared_isBase64_index.isBase64(img)) {
      file.src = img;
    } else if (img.path) {
      file.src = img.path;
    } else if (uni_modules_limeShared_isString_index.isString(img)) {
      file.src = img;
    } else {
      return false;
    }
  ...
  pretty(img) {
    if (!imageHasData(img))
      return "";
    let a, data = img.exifdata, strPretty = "";
    for (a in data) {
      if (data.hasOwnProperty(a)) {
...
function getImageData(img, callback) {
  ...
  if (img.src) {
    if (/^data\:/i.test(img.src)) {
      handleBinaryFile(uni_modules_limeShared_base64ToArrayBuffer_index.base64ToArrayBuffer(img.src));
      ...
      uni_modules_limeShared_pathToBase64_index.pathToBase64(img.src).then((res) => {
        handleBinaryFile(uni_modules_limeShared_base64ToArrayBuffer_index.base64ToArrayBuffer(res));
      });
    }
...
function findEXIFinJPEG(file) {
  ...
  while (offset < length) {
    if (dataView.getUint8(offset) != 255) {
      if (exif.debug)
        console.log("Not a valid marker at offset " + offset + ", found: " + dataView.getUint8(
          offset
        ));
      return false;
    }
...
function findIPTCinJPEG(file) {
  ...
  const isFieldSegmentStart = function(dataView2, offset2) {
...
function readTags(file, tiffStart, dirStart, strings, bigEnd) {
  let entries = file.getUint16(dirStart, !bigEnd), tags = {}, entryOffset, tag;
...
function readTagValue(file, entryOffset, tiffStart, dirStart, bigEnd) {
  let type = file.getUint16(entryOffset + 2, !bigEnd), numValues = file.getUint32(entryOffset + 4, !bigEnd), valueOffset = file.getUint32(entryOffset + 8, !bigEnd) + tiffStart, offset, vals, val, n, numerator, denominator;
  switch (type) {
    case 1:
    case 7:
      if (numValues == 1) {
...
function readEXIFData(file, start) {
  ...
  if (firstIFDOffset < 8) {
    if (exif.debug)
      console.log("Not valid TIFF data! (First offset less than 8)", file.getUint32(
        tiffOffset + 4,
        !bigEnd
      ));
    return false;
...
function findXMPinJPEG(file) {
  ...
  let offset = 2, length = file.byteLength, dom = new DOMParser();
